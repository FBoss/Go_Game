...
//}
//
// template <typename T> constexpr auto chiIsUniform(const T &data, double significance = 0.95) noexcept {
//  int degree_of_freedom = data.size() - 1;
//  auto dist = chi2UniformDistance(data);
//    auto test = chi2Probability(degree_of_freedom, dist);
...
/* Numerical integration method */
inline double Simpson3_8(Ifctn f, double a, double b, int N) {
  int j;
  double l1;
  double h = (b - a) / N;
  double h1 = h / 3.0;
  ...
  return h * sum / 8.0;
    ...
    static double aa1;
    inline double f0(double t) { return pow(t, aa1) * exp(-t); }

    inline double GammaIncomplete_Q(double a, double x) {
      ...
      return 1.0 - Simpson3_8(&f0, 0, y, (int)(y / h)) / tgamma(a); // Gamma_Spouge(a);
    }

    template <typename T> inline double chi2UniformDistance(const T &data) {
      double expected = 0.0;
      double sum = 0.0;
      int k;
      ...
      for (k = 0; k < data.size(); k++) {
        double x = data[k] - expected;
        sum += x * x;
      }
      return sum / expected;
    }

    inline double chi2Probability(int dof, double distance) { return GammaIncomplete_Q(0.5 * dof, 0.5 * distance); }

    template <typename T> inline int chiIsUniform(const T &data, double significance = 0.5) {
      int dof = data.size() - 1;
      double dist = chi2UniformDistance(data);
      auto probability = chi2Probability(dof, dist);
